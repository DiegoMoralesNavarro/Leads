...
public function atualizarSenha($user){
  ...
  if ($this->getsenhaAtual() == '' || $this->getsenhaAtual() == NULL) {
    header("Location: /leads/dashboard/configurar/atualizar-dados?senha=Vazio#senha");
    exit;
  }else{
    ...
    if ($results[0]['senha'] == $md5) {
      ...
      if ($this->getnovaSenha() == '' || $this->getnovaSenha() == NULL) {
        header("Location: /leads/dashboard/configurar/atualizar-dados?senhaAtual=Vazio#senha");
        exit;
      }else{
      ...
      header("Location: /leads/dashboard/configurar/atualizar-dados?senha=Errado#senha");
    exit;
    }
